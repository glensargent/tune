...
import { enumerateDevices, deviceLabel } from '../lib/devices'
...
export default function DeviceSelect(props: DeviceSelectProps) {
  ...
  const [devices] = createResource(
    () => shouldFetch() ? { kind: props.kind, _trigger: fetchTrigger() } : null,
    async (params) => params ? enumerateDevices(params.kind) : [],
  )

  const handleDeviceChange = () => {
    if (shouldFetch()) setFetchTrigger(n => n + 1)
  }
  ...
  createEffect(on(devices, devs => {
    ...
    if (!stillExists) props.onSelect(undefined)
  }))

  const isInput = () => props.kind === 'audioinput'
  const fallback = () => isInput() ? 'Microphone' : 'Speaker'
  const selectedLabel = () => deviceLabel(devices() ?? [], props.selectedId, fallback())
              ...
              <For each={devs()} fallback={
                ...
                {(device, i) => (
                  ...
                  >
                    ...
                    <span class="truncate">{device.label || `${fallback()} ${i() + 1}`}</span>
                  </button>
                )}
              </For>
            )}
